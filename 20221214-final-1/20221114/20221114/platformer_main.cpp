...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		{
			TextOut(hdc, 350, 600, TEXT("'SPACE'를 눌러 점프"), 14);
			TextOut(hdc, 650, 600, TEXT("점프는 발이 땅에 닿을때까지 다시 행할 수 없다"), 26);
		}
		else if (map == 2)
		{
			TextOut(hdc, 150, 500, TEXT("'SHIFT'를 눌러 중력 반전"), 17);
		}
