...
DWORD WINAPI ISCOLLISION(LPVOID param)
{
	while (1)
	{
		...
		{
			if (IsTouchSpike(spikes[i], player))
			{
				p_isdeath = true;
				istouch.x = 50;
			}
		...
		for (size_t i = 0; i < revspikes.size(); i++)
		{
			if (IsTouchRevSpike(revspikes[i], player))
			{
				istouch.x = 50;
			}
			else
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		bloods.push_back({ player.x , player.y , 10 ,10 });
		bloods.push_back({ player.x , player.y , 10 ,10 });
		bloods.push_back({ player.x , player.y , 10 ,10 });
		bloods.push_back({ player.x , player.y , 10 ,10 });
		bloods.push_back({ player.x , player.y , 10 ,10 });

		CreateSpike({150, 620, 50});
		CreateSpike({ 100, 620, 50 });
		CreateSpike({ 200, 620, 50 });

		CreateRevSpike({ 150, 80, 50 });
		CreateRevSpike({ 100, 80, 50 });
		CreateRevSpike({ 200, 80, 50 });

		Create({ 250, 650, 20 ,500 });
		Create({ 250, 50, 20, 500 });
		Create({ 250, 400, 20, 100 });
		Create({ 100, 400, 20 ,100 });
		Create({ 400, 400, 20 ,100 });
	...
	case WM_PAINT:
	{
		...
		for (size_t i = 0; i < spikes.size(); i++)
		{
			DrawSpike(hdc, spikes[i]);
		}
		for (size_t i = 0; i < revspikes.size(); i++)
		{
			DrawRevSpike(hdc, revspikes[i]);
		}
